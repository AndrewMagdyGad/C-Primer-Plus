...
{
    printf("Enter distance travelled in %s: ", mode == METRIC ? "kilometers" : "miles");
    while (scanf("%lf", distance) != 1)
    {
        while (getchar() != '\n')
            ;
        printf("Invalid input. Enter distance travelled in %s: ", mode == METRIC ? "kilometers" : "miles");
    }
    ...
    while (scanf("%lf", fuel) != 1)
    {
        while (getchar() != '\n')
            ;
        printf("Invalid input. Enter fuel consumed in %s: ", mode == METRIC ? "liters" : "gallons");
    }
}
